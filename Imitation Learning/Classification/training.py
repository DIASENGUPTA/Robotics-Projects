...
def train(data_folder, save_path):
    ...
    for epoch in range(nr_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            batch_in, batch_gt, batch_co = batch[0].to(device), infer_action.actions_to_classes(batch[1]).to(device), batch[2].to(device)
            #print(batch_gt.shape)
            #print("Hello")

            batch_out = infer_action1(batch_in,batch_co)
    ...
    plt.title('Loss Plot for Competition Model')
    plt.plot(loss_values)#plotting loss values
    plt.savefig('/projectnb/rlvn/students/ksg25/PythonAPI/examples/projectplot.jpg')#saving the plot
